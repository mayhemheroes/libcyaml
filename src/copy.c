		...
		cyaml_ctx_t *ctx)
{
	...
	if (state->data == NULL) {
		...
		for (f = field; f->key != NULL; f++) {
			if (!(f->value.flags & CYAML_FLAG_OPTIONAL)) {
				return CYAML_ERR_MAPPING_FIELD_MISSING;
			}
		}
		return cyaml__stack_pop(ctx);
	}

	/* Skip optional mapping fields */
	while (field->key != NULL && field->value.flags & CYAML_FLAG_OPTIONAL) {
